...
PROCESS_THREAD(sensor_node, ev, data) 
{
    ...
    while (1) {
        ...
        uint8_t is_node_reachable = NETSTACK_ROUTING.node_is_reachable();//se va a fijar si el nodo ya pertenece al nodo raíz
        ...
        if (is_node_reachable && got_server_address) { 
            LOG_INFO("Sending request %u to "); //enviar info al nodo # 
            ...
            snprintf(buffer, sizeof(buffer), "temp: %.2f, humidity:%.2f", temp_reading, hum_reading); //dar formato a datos a enviar. Guardar en buffer, indicar el tamaño del mismo y el mensaje
            simple_udp_sendto(&connection, buffer, strlen(buffer), &server_address);//enviar info a través de variable connection
